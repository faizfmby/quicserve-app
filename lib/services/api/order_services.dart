...
class OrderServices {
  ...
  Future<Map<String, dynamic>> createOrder() async {
    try {
      ...
      final endpoint =
          '${ApiEndpoints.withCompany(companySlug, ApiEndpoints.orders)}/create?staffID=$staffID';
  ...
  Future<List<Order>> fetchOrderDetails() async {
    try {
      ...
      final response = await _apiService.get(ApiEndpoints.withCompany(companySlug, ApiEndpoints.orders));
      ...
      if (response['success'] == true) {
        ...
        throw Exception(
            response['message']?.toString() ?? 'Failed to load order');
      }
    } catch (e) {
  ...
  Future<List<Order>> fetchSelectedOrder({required String orderId}) async {
    try {
      ...
      final response = await _apiService.get('${ApiEndpoints.withCompany(companySlug, ApiEndpoints.orders)}/$orderId');
      ...
      if (response['success'] == true) {
        ...
        if (data == null) {
          print(
              'Warning: No data field for selected order in response: $response');
          return [];
        }
        ...
        try {
          ...
          return [order]; // Return as a single-item list
        } catch (e) {
        ...
        throw Exception(
            'Failed to load selected order: ${response['message'] ?? 'Unknown error'}');
      }
    } catch (e) {
  ...
  Future<List<Order>> fetchOrderByStatus({required String orderStatus}) async {
    try {
      ...
      final response =
          await _apiService.get('${ApiEndpoints.withCompany(companySlug, ApiEndpoints.orders)}/status/$orderStatus');
      ...
      if (response['success'] == true) {
        ...
        throw Exception(
            'Failed to load order: ${response['message'] ?? 'Unknown error'}');
      }
    } catch (e) {
  ...
  Future<Map<String, dynamic>> saveOrder({
    ...
    try {
      ...
      final response =
          await _apiService.post('${ApiEndpoints.withCompany(companySlug, ApiEndpoints.orders)}/$orderID/update', {});
      ...
      if (message == 'order status is updated') {
        ...
        throw Exception(
            response['message']?.toString() ?? 'Failed to save order');
      }
    } catch (e) {
  ...
  Future<Map<String, dynamic>> paidOrder({
    ...
    try {
      ...
      final response = await _apiService.put(
        '${ApiEndpoints.withCompany(companySlug, ApiEndpoints.orders)}/$orderID/paid',
        {
          'paymentID': paymentID,
        },
      );
      ...
      if (message == 'order marked as paid' || message == 'success') {
        ...
        throw Exception(
            response['message']?.toString() ?? 'Failed to make payment');
      }
    } catch (e) {
  ...
  Future<Map<String, dynamic>> deleteOrder(String orderID) async {
    try {
      ...
      final response =
          await _apiService.delete('${ApiEndpoints.withCompany(companySlug, ApiEndpoints.orders)}/$orderID/delete');
  ...
  Future<Map<String, dynamic>> cancelOrder({
    ...
    try {
      ...
      final response = await _apiService.put(
        '${ApiEndpoints.withCompany(companySlug, ApiEndpoints.orders)}/$orderID/cancel',
      ...
      if (message != 'success' && message != 'order canceled') {
        throw Exception(
            response['message']?.toString() ?? 'Failed to delete opened order');
      }
